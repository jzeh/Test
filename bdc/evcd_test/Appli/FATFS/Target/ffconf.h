...
/* CubeMX 가 user_diskio.c 의 #if _USE_WRITE / #if _USE_IOCTL 가드를 매번 복구함.
 *  → 이 매크로들을 1 로 강제 정의해서 가드가 복구돼도 USER_write/ioctl 가 NULL 안 되게 함.
 *  → FatFs 0.14+ 에서는 의미 없지만 (코어가 사용 안 함), user_diskio.c 의 컴파일 가드만 통과시킴 */
#define _USE_WRITE   1
#define _USE_IOCTL   1
