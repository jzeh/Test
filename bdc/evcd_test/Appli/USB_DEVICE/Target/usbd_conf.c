...
void HAL_PCD_MspInit(PCD_HandleTypeDef* pcdHandle)
{
  ...
  /* USB 클럭원(HSI48) 명시적 활성화.
   *   Appli 는 자체 SystemClock_Config 가 없어 Boot 클럭을 상속하지만,
   *   USB enumeration 안정화를 위해 HSI48 ON 을 여기서 보장한다.
   *   (이미 켜져 있으면 HAL_RCC_OscConfig 가 OK 로 통과) */
  {
    RCC_OscInitTypeDef oscHsi48 = {0};
    oscHsi48.OscillatorType = RCC_OSCILLATORTYPE_HSI48;
    oscHsi48.HSI48State     = RCC_HSI48_ON;
    if (HAL_RCC_OscConfig(&oscHsi48) != HAL_OK)
    {
      Error_Handler();
    }
  }
  ...
  */
  ...
  /* CRS — USB SOF 로 HSI48 을 자동 트리밍하여 USB FS 정밀도(±0.25%) 확보.
   *   CRS 미설정 시 HSI48 free-run 오차가 규격을 벗어나
   *   enumeration("장치 설명자 요청 실패")이 실패할 수 있다. */
  {
    RCC_CRSInitTypeDef crs = {0};
    __HAL_RCC_CRS_CLK_ENABLE();
    crs.Prescaler             = RCC_CRS_SYNC_DIV1;
    crs.Source                = RCC_CRS_SYNC_SOURCE_USB_OTG_FS;
...
#endif /* USE_HAL_PCD_REGISTER_CALLBACKS */
{
  ...
  /* 연결/해제 감지는 USB_MSC_PollConnection() (dev_state 폴링) 으로 일원화. */
