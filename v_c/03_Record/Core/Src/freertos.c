...
extern bool g_bUsingKM;//NEW HSM ONLY
...
//	uint32_t	ret		= 0;
  ...
  osThreadDef(defaultTask, StartDefaultTask, osPriorityNormal, 0, (1024+1024)*2);
	...
	for(;;)
	{
		...
		switch( g_eMainState )
		{
                ...
                if ( LoadRunRepro_Info() == TRUE )
                {
                              ...
                              g_pstRunReproInfo[g_ucCommRDBIIdx].ucRetryCount <= 2 )
                    {
                        ...
                        g_eMainState = eMain_JUMP_to_ECU_Upgrade;
                              ...
                              g_pstRunReproInfo[g_ucCommRDBIIdx].ucRetryCount < 0xFF ) // 0xFF는 성공 
                    {
                        DeleteRunRepro_Info();
                        g_eMainState = eMain_Run;
            ...
            case eMain_LoadFile_RDBI:
                ...
                {
                    BYTE arrSendData[14];
                    uint32_t nProtocolID = ISO15765; // ISO14230이 필요하면 RDBI에 프로토콜 종류기입 해야함.
                ...
                FL_GitPassThruDisconnect(NULL, PACKET_INTER_ANALYSIS, 0);
                    ...
                    for ( int i=0; i<g_pstRDBIInfo[0].unECUCount; i++ )
                    {
                        ...
                        if ( bFoundPartNo == TRUE )
                        {
                            ...
                            {
                                if ( g_pstRunReproInfo[i].ucRetryCount == 0xFF ) // 0xFF는 성공
                                {
                                    LED_BLUE_ON;
                            ...
                            {
                                // 2024/01/17 김진호 책임 요청(개발버전으로) : 버전이 달라도 업그레이드 
                                bCreateRunReproFile = g_pstRDBIInfo[i].bIsUpgradeTarget = TRUE;
/*
                                // 미대상 
...
*/
                    ...
                    {
                        CreateRunRepro_Info();  // RunRepor.ini생성
                g_eMainState = eMain_JUMP_to_ECU_Upgrade;
                    }
                    else
                    {
			...
			case eMain_Run :
			{
				...
				{
					MQTTPacketSend(MQTT_MESSAGE_TYPE_HEALTH_CHECK, "1\0", 1);
...
#endif
	if( g_ucBtConnected == 0 && g_iUSBConnected != USBD_STATE_CONFIGURED)
	{
...
#if(!VCI_III_ASING_MODE)
#if defined(FEATURE_DETERMINE_ECU_TO_UPGRADE)
		  	if( Get_TmrDelta( Get_Tmr(), m_unBtConnCheck_Timer ) > nLEDTimeOut )
#else
		  	if( Get_TmrDelta( Get_Tmr(), m_unBtConnCheck_Timer ) > 180000 )
#endif
			{
#if defined(FEATURE_DETERMINE_ECU_TO_UPGRADE)
                nLEDTimeOut = 30000;
			  	...
			  	LED_ALL_OFF;
				LED_RED_ON;
			  	LED_SetState(eLED_NOTI, 30000, 100);
