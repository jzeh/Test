...
ITStatus CAN_GetITStatus(CAN_TypeDef* CANx, uint32_t CAN_IT)
{
 ...
 if(CAN_IT == CAN_NO_TRANSMIT)
 {
    itstatus = CheckITStatus(CANx->TSR, CAN_TSR_TME0|CAN_TSR_TME1|CAN_TSR_TME2);  
     /* // jkc_0230327_BEGIN -- check
     if((CANx->TSR & CAN_TSR_TME) == (CAN_TSR_TME0 | CAN_TSR_TME1 | CAN_TSR_TME2))
         itstatus = SET;
     else
         itstatus = RESET;
     */ // jkc_0230327_END -- check
 }
 else if((CANx->IER & CAN_IT) != RESET)  /* check the interrupt enable bit */
 {
