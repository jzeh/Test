...
uint16_t VCP_DataTxByte   (uint8_t ch);
...
uint16_t VCP_DataTx (void)
{
  if (linecoding.datatype == 7)
  {
	    ...
	    APP_Rx_Buffer[APP_Rx_ptr_in] = USART_ReceiveData(UART8) & 0x7F;
	  }
	  else {
    	APP_Rx_Buffer[APP_Rx_ptr_in] = USART_ReceiveData(UART7) & 0x7F;
    }
  }
  else if (linecoding.datatype == 8)
  {
		if(g_bHYPERTECSelftestFlag == true) {
	    APP_Rx_Buffer[APP_Rx_ptr_in] = USART_ReceiveData(UART8);
		}
		else {
    	APP_Rx_Buffer[APP_Rx_ptr_in] = USART_ReceiveData(UART7);
    }
  }
...
uint16_t VCP_DataTxByte (uint8_t ch)
{
  if (linecoding.datatype == 7)
  {
    APP_Rx_Buffer[APP_Rx_ptr_in] = ch & 0x7F;
  }
  else if (linecoding.datatype == 8)
  {
 ...
 */
#define EVAL_COM1                        UART7
#define EVAL_COM1_CLK                    RCC_APB1Periph_UART7
#define EVAL_COM1_TX_PIN                 GPIO_Pin_8
#define EVAL_COM1_TX_GPIO_PORT           GPIOE
#define EVAL_COM1_TX_GPIO_CLK            RCC_AHB1Periph_GPIOE
#define EVAL_COM1_TX_SOURCE              GPIO_PinSource8
#define EVAL_COM1_TX_AF                  GPIO_AF_UART7
#define EVAL_COM1_RX_PIN                 GPIO_Pin_7
#define EVAL_COM1_RX_GPIO_PORT           GPIOE
#define EVAL_COM1_RX_GPIO_CLK            RCC_AHB1Periph_GPIOE
#define EVAL_COM1_RX_SOURCE              GPIO_PinSource7
#define EVAL_COM1_RX_AF                  GPIO_AF_UART7
#define EVAL_COM1_IRQn                   UART7_IRQn
...
void STM_EVAL_COMInit(COM_TypeDef COM, USART_InitTypeDef* USART_InitStruct)
{
  ...
  /* Enable USART */
  HalDrvUartIOCtrl(eUART_IO_Port_ENABLE, (int)COM_USART[COM], NULL, 0, HAL_ENABLE);
}
...
{
    if ( HalUartGetFlagStatus(g_stUart7.pUARTreg, HAL_USART_IT_RXNE) != HAL_RESET )
    {
    ...
    if(HalUartGetFlagStatus(g_stUart7.pUARTreg, HAL_USART_FLAG_ORE) != HAL_RESET) {
    {
