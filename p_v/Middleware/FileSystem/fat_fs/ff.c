...
)
{
	...
	for (;;) {
		...
		if (IsDBCS1(w)) {				/* Check if it is a DBC 1st byte (always false on SBCS cfg) */
			...
			//w = (w << 8) + b;			/* Create a DBC */	Warning[Pe111]: statement is unreachable Z:\VCI2\New\DCSP\STM32F4XX\Premium_Board\drv\fat_fs\ff.c 1646
		}
...
)
{
	...
	while (n < len - 1) {			/* Read bytes until buffer gets filled */
		...
		*p++ = c;
		n++;
		//if (c == '\n') break;		/* Break on EOL */	//131022 LWH 스트링으로 쓰지 않기 위해 막음
	}
	*p = 0;
