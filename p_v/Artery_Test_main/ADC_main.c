...
/** @addtogroup 435_ADC_voltage_monitoring ADC_voltage_monitoring
  * @{
  */

#define ADC_BUFFER_SIZE 10
__IO uint16_t adc1_ordinary_valuetab[ADC_BUFFER_SIZE] = {0};
...
static void dma_config(void)
{
  ...
  dma_init_struct.peripheral_base_addr = (uint32_t)&(ADC2->odt);
  ...
  dmamux_init(DMA2MUX_CHANNEL3, DMAMUX_DMAREQ_ID_ADC2);
...
static void adc_config(void)
{
  ...
  adc_common_struct.sampling_interval = ADC_SAMPLING_INTERVAL_20CYCLES;//ADC_SAMPLING_INTERVAL_5CYCLES;
  ...
  adc_base_struct.sequence_mode = TRUE; //TRUE;
  ...
  adc_ordinary_channel_set(ADC2, ADC_CHANNEL_2, 1, ADC_SAMPLETIME_640_5);
//  adc_ordinary_channel_set(ADC2, ADC_CHANNEL_5, 2, ADC_SAMPLETIME_47_5);
//  adc_ordinary_channel_set(ADC2, ADC_CHANNEL_6, 3, ADC_SAMPLETIME_47_5);
  ...
  /*
  adc_voltage_monitor_threshold_value_set(ADC2, 0x100, 0x000);
  adc_voltage_monitor_single_channel_select(ADC2, ADC_CHANNEL_2);
  adc_voltage_monitor_enable(ADC2, ADC_VMONITOR_SINGLE_ORDINARY);
  */
  ...
  //adc_interrupt_enable(ADC2, ADC_VMOR_INT, TRUE);
...
int main(void)
{
  ...
  system_clock_config_GIT();
   ...
   //crm_periph_reset(CRM_DMA1_PERIPH_CLOCK , TRUE);
   crm_periph_reset(CRM_DMA2_PERIPH_CLOCK , TRUE);
  ...
  while(1)
  {
    ...
    if((adc1_overflow_flag != 0) || (vmor_flag_index != 0))
    {
      ...
      printf("out of range:adc1_channel_1 value is = 0x%x!\r\n", adc1_ordinary_valuetab[1]);
    }
#endif
    index++;
    index++;
...
void ADC1_2_3_IRQHandler(void)
{
  ...
  if(adc_flag_get(ADC2, ADC_VMOR_FLAG) != RESET)
  {
    adc_flag_clear(ADC2, ADC_VMOR_FLAG);
    vmor_flag_index++;
  }
}
