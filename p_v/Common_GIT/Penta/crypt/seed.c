...
int SEED_CBC_Encrypt( OUT unsigned char *pbszCipherText, size_t *out_len, IN const unsigned char *pbszPlainText, IN size_t nPlainTextLen, IN const unsigned char *pbszUserKey, IN unsigned char *pbszIV, size_t pad );
int SEED_CBC_Decrypt(OUT unsigned char *result, size_t *out_len, IN const unsigned char *pbszCipherText, IN size_t nCipherTextLen, IN const unsigned char *pbszUserKey, int mode, IN unsigned char *pbszIV, size_t pad);
int SEED_CFB_Encrypt(unsigned char *in, size_t inLen, DWORD *key, unsigned char *iv, size_t blockLen);
int SEED_CFB_Decrypt(unsigned char *in, size_t inLen, DWORD *key, unsigned char *iv, size_t blockLen);
...
unsigned int* chartoint32_for_SEED_CBC( IN BYTE *in, IN int nLen )
{
#if defined(DUKPT_SERVER) || defined (DUKPT_CLIENT_ENCRYPT_ALLOC)
	unsigned int *data = NULL;
#else
	unsigned int data[INLENMAX];
#endif
...
#if defined(DUKPT_SERVER) || defined (DUKPT_CLIENT_ENCRYPT_ALLOC)
	data = (unsigned char*) malloc(sizeof(unsigned char) * nLen);
#endif
...
int SEED_CBC_init( OUT KISA_SEED_INFO *pInfo, IN KISA_ENC_DEC enc, IN const unsigned char *pbszUserKey, IN unsigned char *pbszIV )
{
		...
		NULL == pbszIV )
	{
		if(enc == KISA_ENCRYPT)
		{
			return DAMO_CRYPT_ERR_SEED_ENC_NULL_POINTER;
		}
		else
		{
			return DAMO_CRYPT_ERR_SEED_DEC_NULL_POINTER;
		}
	}
...
{
	int nCurrentCount = SeedBlockSize;
	DWORD *pdwXOR;
	...
	if( KISA_ENCRYPT == pInfo->encrypt )
	{
		...
		while( nCurrentCount <= inLen )
		{
			...
			KISA_SEED_Encrypt_Block_forCBC( (unsigned int*)out, (unsigned int*)out, &pInfo->seed_key );
			pdwXOR = out;
		...
		*outLen = nCurrentCount - SeedBlockSize;
		pInfo->buffer_length = inLen - *outLen;

		memcpy( pInfo->ivec, pdwXOR, SeedBlockSize );
	...
	else
	{
		...
		while( nCurrentCount <= inLen )
		{
			KISA_SEED_Decrypt_Block_forCBC( (unsigned int*)in, (unsigned int*)out, &pInfo->seed_key );
		...
		*outLen = nCurrentCount - SeedBlockSize;
		memcpy( pInfo->ivec, pdwXOR, SeedBlockSize );
		memcpy( pInfo->cbc_last_block, out - BLOCK_SIZE_SEED_INT, SeedBlockSize );
...
int SEED_CBC_Close( OUT KISA_SEED_INFO *pInfo, IN DWORD *out, IN int *outLen, IN int pad )
{
	...
	*outLen = 0;
	...
	if( KISA_ENCRYPT == pInfo->encrypt )
	{
		nPaddngLeng = SeedBlockSize - pInfo->buffer_length;

		for( i = pInfo->buffer_length; i<SeedBlockSize; i++ )
		{
			...
			{
				return DAMO_CRYPT_ERR_SEED_ENC_INVALID_PAD;
			}			
		}
		
		BLOCK_XOR_CBC( pInfo->cbc_buffer, pInfo->cbc_buffer, pInfo->ivec );
		
		KISA_SEED_Encrypt_Block_forCBC( (unsigned int*)pInfo->cbc_buffer, (unsigned int*)out, &pInfo->seed_key );
		out += BLOCK_SIZE_SEED_INT;
		*outLen = SeedBlockSize;
	}
	else
	{
		if(pad == DAMO_PKCS7_PADDING)
		{
			nPaddngLeng = ((BYTE*)pInfo->cbc_last_block)[SeedBlockSize-1];
			if( nPaddngLeng > 0 && nPaddngLeng <= SeedBlockSize )
			{
				...
				*outLen = nPaddngLeng;
				...
				return DAMO_CRYPT_ERR_SEED_DEC_GEOMJEUNG_PAD;
		}
		else if(pad == DAMO_ZERO_PADDING)
		{
			for ( i = 1 ; i <= SeedBlockSize ; i++)
			{
				if( ((BYTE*)pInfo->cbc_last_block)[SeedBlockSize-i] != 0x00 )
				{
					*outLen = i - 1;
					return DAMO_CRYPT_SUCCESS;
				}
		...
		{
          return DAMO_CRYPT_ERR_SEED_DEC_INVALID_PAD;
		}
	}
...
int DAMO_CRYPT_SEED_Encrypt_Core(unsigned char *out, size_t *outLen, const unsigned char *in, size_t inLen, const unsigned char *key, int mode, unsigned char *iv, size_t pad)
{
	...
	switch(mode)
	{
	  case CBC_MODE:
	  {
        ret = SEED_CBC_Encrypt(out, outLen, in, inLen, key, iv, pad);
	  }
	  break;
	  case CFB_MODE:
	  {
        ret = SEED_CFB_Encrypt(out, inLen, pdwRoundKey, iv, SeedBlockSize);
		*outLen = inLen;
	  }
	  break;
	}	
...
int DAMO_CRYPT_SEED_Decrypt_Core(unsigned char *out, size_t *outLen, const unsigned char *in, size_t inLen, const unsigned char *key, int mode, unsigned char *iv, size_t pad)
{
	...
	switch(mode)
	{
	  case CBC_MODE:
	  {
        ret = SEED_CBC_Decrypt(out, outLen, in, inLen, key, mode, iv, pad);
	  }
	  break;
	  case CFB_MODE:
	  {
        ret = SEED_CFB_Decrypt(out, inLen, pdwRoundKey, iv, SeedBlockSize);
		*outLen = inLen;
	  }
	  break;
	}	
...
int SEED_CFB_Encrypt(BYTE *in, size_t inLen, DWORD *key, BYTE *iv, size_t blockLen)
{
	while(1 <= inLen)
	{
		SeedEncrypt(iv, key);
...
int SEED_CBC_Encrypt( OUT unsigned char *pbszCipherText, size_t *out_len, IN const unsigned char *pbszPlainText, IN size_t nPlainTextLen, IN const unsigned char *pbszUserKey, IN unsigned char *pbszIV, size_t pad )
{
  ...
  if(pad == DAMO_PKCS7_PADDING)
  {
     nPlainTextPadding = (SeedBlockSize - (nPlainTextLen % SeedBlockSize));
  }
  else if(pad == DAMO_ZERO_PADDING)
  {
	  if (nPlainTextLen % SeedBlockSize == 0)
	  {
		  nPlainTextPadding = 0;
	  }
	  else
  ...
  newpbszPlainText = (unsigned char*) malloc(sizeof(unsigned char) * (nPlainTextLen + nPlainTextPadding));
	...
	ret = SEED_CBC_init( &info, KISA_ENCRYPT, pbszUserKey, pbszIV );
	...
	data = (unsigned int*)chartoint32_for_SEED_CBC(newpbszPlainText, nPlainTextLen);
	ret = SEED_CBC_Process( &info, (unsigned long*)data, nPlainTextLen, (unsigned long*)outbuf, &nRetOutLeng );
    if(ret != DAMO_CRYPT_SUCCESS)
	{
#if defined(DUKPT_SERVER) || defined (DUKPT_CLIENT_ENCRYPT_ALLOC)
		free(newpbszPlainText);
	...
	ret = SEED_CBC_Close( &info, (unsigned long*)outbuf + (nRetOutLeng)/4, &nPaddingLeng, pad );
	if(ret != DAMO_CRYPT_SUCCESS)
	{
#if defined(DUKPT_SERVER) || defined (DUKPT_CLIENT_ENCRYPT_ALLOC)
		free(newpbszPlainText);
	...
	cdata = int32tochar_for_SEED_CBC((unsigned long*)outbuf, nRetOutLeng + nPaddingLeng);
	memcpy(pbszCipherText, cdata, nRetOutLeng + nPaddingLeng);

	*out_len = nRetOutLeng + nPaddingLeng;
...
int SEED_CBC_Decrypt(OUT unsigned char *result, size_t *out_len, IN const unsigned char *pbszCipherText, IN size_t nCipherTextLen, IN const unsigned char *pbszUserKey, int mode, IN unsigned char *pbszIV, size_t pad)
{
	...
	if ((nCipherTextLen % SeedBlockSize) > 0)
	{
		return 0;
	}
  ...
  pbszPlainText = malloc(sizeof(unsigned char) * (nCipherTextLen));
	...
	ret = SEED_CBC_init( &info, KISA_DECRYPT, pbszUserKey, pbszIV );
	if(ret != DAMO_CRYPT_SUCCESS)
	{
#if defined(DUKPT_SERVER) || defined (DUKPT_CLIENT_ENCRYPT_ALLOC)
	...
	data = (unsigned int*)chartoint32_for_SEED_CBC(newpbszCipherText, nCipherTextLen);
	ret = SEED_CBC_Process( &info, (unsigned long*)data, nCipherTextLen, (unsigned long*)outbuf, &nRetOutLeng );
	if(ret != DAMO_CRYPT_SUCCESS)
	{
#if defined(DUKPT_SERVER) || defined (DUKPT_CLIENT_ENCRYPT_ALLOC)
       free(newpbszCipherText);
