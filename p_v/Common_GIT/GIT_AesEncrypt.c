...
#define KE_ROTWORD_NEW(x) (((x) << 24) | ((x) >> 8))
...
* AES - CBC
*******************/
int aes_encrypt_cbc(const BYTE in[], size_t in_len, BYTE out[], const DWORD key[], int keysize, const BYTE iv[])
{
...
int aes_encrypt_cbc_new(const BYTE in[], size_t in_len, BYTE out[], const DWORD key[], int keysize, const BYTE iv[])
{
	BYTE buf_in[AES_BLOCK_SIZE], buf_out[AES_BLOCK_SIZE], iv_buf[AES_BLOCK_SIZE];
	int blocks, idx;
...
//		printf(" - %d\n", idx);
		aes_encrypt_new(buf_in, buf_out, key, keysize);
...
int aes_encrypt_cbc_mac(const BYTE in[], size_t in_len, BYTE out[], const DWORD key[], int keysize, const BYTE iv[])
{
	BYTE buf_in[AES_BLOCK_SIZE], buf_out[AES_BLOCK_SIZE], iv_buf[AES_BLOCK_SIZE];
	int blocks, idx;
...
int aes_decrypt_cbc_new(const BYTE in[], size_t in_len, BYTE out[], const DWORD key[], int keysize, const BYTE iv[])
{
	...
	for (idx = 0; idx < blocks; idx++) {
		memcpy(buf_in, &in[idx * AES_BLOCK_SIZE], AES_BLOCK_SIZE);
		aes_decrypt_new(buf_in, buf_out, key, keysize);
		xor_buf(iv_buf, buf_out, AES_BLOCK_SIZE);
		memcpy(&out[idx * AES_BLOCK_SIZE], buf_out, AES_BLOCK_SIZE);
		memcpy(iv_buf, buf_in, AES_BLOCK_SIZE);
	}
...
void aes_encrypt_ctr(const BYTE in[], size_t in_len, BYTE out[], const DWORD key[], int keysize, const BYTE iv[])
{
...
void aes_decrypt_ctr(const BYTE in[], size_t in_len, BYTE out[], const DWORD key[], int keysize, const BYTE iv[])
{
...
int aes_encrypt_ccm(const BYTE payload[], DWORD payload_len, const BYTE assoc[], unsigned short assoc_len,
                    const BYTE nonce[], unsigned short nonce_len, BYTE out[], DWORD *out_len,
                    DWORD mac_len, const BYTE key_str[], int keysize)
{
...
int aes_decrypt_ccm(const BYTE ciphertext[], DWORD ciphertext_len, const BYTE assoc[], unsigned short assoc_len,
                    const BYTE nonce[], unsigned short nonce_len, BYTE plaintext[], DWORD *plaintext_len,
                    DWORD mac_len, int *mac_auth, const BYTE key_str[], int keysize)
{
...
void aes_key_setup(const BYTE key[], DWORD w[], int keysize)
{
	int Nb=4,Nr,Nk,idx;
	DWORD temp;
	DWORD Rcon[] = {0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000, 0x20000000,
...
void aes_key_setup_new(const BYTE key[], DWORD w[], int keysize)
{
	int Nb=4,Nr,Nk,idx;
	DWORD temp;
	DWORD Rcon[] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20,
	...
	for (idx = Nk; idx < Nb * (Nr + 1); ++idx) {
		...
		if ((idx % Nk) == 0) {
			...
			bbb = SubWord(KE_ROTWORD_NEW(temp));
			ccc = Rcon[(idx - 1) / Nk];
			temp = bbb ^ ccc;
		}
		else if (Nk > 6 && (idx % Nk) == 4) {
...
void AddRoundKey_new(BYTE state[][4], const DWORD w[])
{
	BYTE subkey[4];
...
void aes_encrypt(const BYTE in[], BYTE out[], const DWORD key[], int keysize)
{
	BYTE state[4][4];
...
void aes_encrypt_new(const BYTE in[], BYTE out[], const DWORD key[], int keysize)
{
	BYTE state[4][4];
...
//	printf("\n\n");
	...
	// Perform the necessary number of rounds. The round key is added first.
	// The last round does not perform the MixColumns step.
	AddRoundKey_new(state,&key[0]);

	SubBytes(state); ShiftRows(state); MixColumns(state); AddRoundKey_new(state,&key[4]);
	SubBytes(state); ShiftRows(state); MixColumns(state); AddRoundKey_new(state,&key[8]);
	SubBytes(state); ShiftRows(state); MixColumns(state); AddRoundKey_new(state,&key[12]);
	SubBytes(state); ShiftRows(state); MixColumns(state); AddRoundKey_new(state,&key[16]);
	SubBytes(state); ShiftRows(state); MixColumns(state); AddRoundKey_new(state,&key[20]);
	SubBytes(state); ShiftRows(state); MixColumns(state); AddRoundKey_new(state,&key[24]);
	SubBytes(state); ShiftRows(state); MixColumns(state); AddRoundKey_new(state,&key[28]);
	SubBytes(state); ShiftRows(state); MixColumns(state); AddRoundKey_new(state,&key[32]);
	SubBytes(state); ShiftRows(state); MixColumns(state); AddRoundKey_new(state,&key[36]);
	if (keysize != 128) {
		SubBytes(state); ShiftRows(state); MixColumns(state); AddRoundKey_new(state,&key[40]);
		SubBytes(state); ShiftRows(state); MixColumns(state); AddRoundKey_new(state,&key[44]);
		if (keysize != 192) {
			SubBytes(state); ShiftRows(state); MixColumns(state); AddRoundKey_new(state,&key[48]);
			SubBytes(state); ShiftRows(state); MixColumns(state); AddRoundKey_new(state,&key[52]);
			SubBytes(state); ShiftRows(state); AddRoundKey_new(state,&key[56]);
		}
		else {
			SubBytes(state); ShiftRows(state); AddRoundKey_new(state,&key[48]);
		}
	}
	else {
		SubBytes(state); ShiftRows(state); AddRoundKey_new(state,&key[40]);
	}
...
void aes_decrypt(const BYTE in[], BYTE out[], const DWORD key[], int keysize)
{
	BYTE state[4][4];
...
void aes_decrypt_new(const BYTE in[], BYTE out[], const DWORD key[], int keysize)
{
	BYTE state[4][4];
	...
	if (keysize > 128) {
		if (keysize > 192) {
			AddRoundKey_new(state,&key[56]);
			InvShiftRows(state);InvSubBytes(state);AddRoundKey_new(state,&key[52]);InvMixColumns(state);
			InvShiftRows(state);InvSubBytes(state);AddRoundKey_new(state,&key[48]);InvMixColumns(state);
		}
		else {
			AddRoundKey_new(state,&key[48]);
		}
		InvShiftRows(state);InvSubBytes(state);AddRoundKey_new(state,&key[44]);InvMixColumns(state);
		InvShiftRows(state);InvSubBytes(state);AddRoundKey_new(state,&key[40]);InvMixColumns(state);
	}
	else {
		AddRoundKey_new(state,&key[40]);
	}
	InvShiftRows(state);InvSubBytes(state);AddRoundKey_new(state,&key[36]);InvMixColumns(state);
	InvShiftRows(state);InvSubBytes(state);AddRoundKey_new(state,&key[32]);InvMixColumns(state);
	InvShiftRows(state);InvSubBytes(state);AddRoundKey_new(state,&key[28]);InvMixColumns(state);
	InvShiftRows(state);InvSubBytes(state);AddRoundKey_new(state,&key[24]);InvMixColumns(state);
	InvShiftRows(state);InvSubBytes(state);AddRoundKey_new(state,&key[20]);InvMixColumns(state);
	InvShiftRows(state);InvSubBytes(state);AddRoundKey_new(state,&key[16]);InvMixColumns(state);
	InvShiftRows(state);InvSubBytes(state);AddRoundKey_new(state,&key[12]);InvMixColumns(state);
	InvShiftRows(state);InvSubBytes(state);AddRoundKey_new(state,&key[8]);InvMixColumns(state);
	InvShiftRows(state);InvSubBytes(state);AddRoundKey_new(state,&key[4]);InvMixColumns(state);
	InvShiftRows(state);InvSubBytes(state);AddRoundKey_new(state,&key[0]);
