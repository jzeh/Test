...
void sFLASH_Init(void)
{
    ...
    SPI_InitStructure.SPI_CPOL = HAL_SPI_CPOL_High;
    SPI_InitStructure.SPI_CPHA = HAL_SPI_CPHA_2Edge;
    SPI_InitStructure.SPI_NSS = HAL_SPI_NSS_Soft;
#if defined(STM32F427X)
    SPI_InitStructure.SPI_BaudRatePrescaler = HAL_SPI_BaudRatePrescaler_2;
#elif defined(AT32F435VMT7)
    SPI_InitStructure.SPI_BaudRatePrescaler = HAL_SPI_BaudRatePrescaler_4;
#endif

    SPI_InitStructure.SPI_FirstBit = HAL_SPI_FirstBit_MSB;
    SPI_InitStructure.SPI_CRCPolynomial = 7;

    HalDrvSPIIOCtrl(eSPI_IO_Init, (int)sFLASH_SPI, (char*)&SPI_InitStructure, sizeof(stHalSPI_InitTypeDef), 0);
    
    /*!< Enable the sFLASH_SPI  */
    HalDrvSPIIOCtrl(eSPI_IO_Enable, (int)sFLASH_SPI, NULL, 0, HAL_ENABLE);
}
...
uint32_t sFLASH_ReadID(void)
{
    ...
    // micron 64Mbit: sFLASH_N25Q064A_ID        0x20BA17				// Micron f?
    // micron 64Mbit: sFLASH_N25Q064A_ID        0x20BA18				// Micron f?				// Micron, 2017년 12월 제작된 보드는 ID가 바뀌었다.
    //printf("SFlash ID: 0x%06x\n", Temp);
...
uint8_t sFLASH_SendByte(uint8_t byte)
{
    ...
    while( HalDrvSPIIOCtrl(eSPI_IO_GetFlagStatus, (int)sFLASH_SPI, NULL, 0, HAL_SPI_I2S_FLAG_TXE) == HAL_RESET );


    /*!< Send byte through the SPI1 peripheral */
    HalDrvSPIWrite((int)sFLASH_SPI, 0, (char*)&byte, sizeof(unsigned char), 0);

    /*!< Wait to receive a byte */
    while( HalDrvSPIIOCtrl(eSPI_IO_GetFlagStatus, (int)sFLASH_SPI, NULL, 0, HAL_SPI_I2S_FLAG_RXNE) == HAL_RESET );

    /*!< Return the byte read from the SPI bus */
    return HalDrvSPIRead((int)sFLASH_SPI, 0, NULL, 0, 0);
}
...
uint16_t sFLASH_SendHalfWord(uint16_t HalfWord)
{
    ...
    HalDrvSPIWrite((int)sFLASH_SPI, 0, (char*)&HalfWord, sizeof(unsigned short), 0);
...
//    printf("flashstatus: 0x%02x\r", flashstatus);
  }
...
void sFLASH_WriteStatusReg(unsigned int data)
{
//  uint8_t flashstatus = 0;

//	printf("sFLASH_WriteStatusReg() 0x%x\n", data);
  /*!< Select the FLASH: Chip Select low */
  sFLASH_CS_LOW();
...
uint16_t sFLASH_ReadStatusReg(void)
{
  uint8_t flashstatus = 0;
...
//	printf("sFLASH_ReadStatusReg(): 0x%x\n", flashstatus);

  return flashstatus;
}

void sFLASH_ReadConfiguration(void)
{
...
//  printf("sFLASH_ReadConfiguration(): 0x%x\n", flashstatus);
}

void sFLASH_WriteBlockProtection(void)
{
  /*!< Enable the write access to the FLASH */
  sFLASH_WriteEnable();
...
*
************************************************************************/
void sFLASH_SST25ResetWriteProtection(void)
{
	sFLASH_CS_LOW();

  sFLASH_SendByte(0x50);
...
void sFLASH_LowLevel_Init(void)
{
    ...
    HalDrvGpioIOCtrl(eGPIO_IO_AF_MAPPING, sFLASH_SPI_SCK_PIN, NULL, 0, HAL_GPIO_AF_SPI4);
    HalDrvGpioIOCtrl(eGPIO_IO_AF_MAPPING, sFLASH_SPI_MISO_PIN, NULL, 0, HAL_GPIO_AF_SPI4);
    HalDrvGpioIOCtrl(eGPIO_IO_AF_MAPPING, sFLASH_SPI_MOSI_PIN, NULL, 0, HAL_GPIO_AF_SPI4);
...
void sFLASH_LowLevel_DeInit(void)
{
  ...
  HalDrvSPIIOCtrl(eSPI_IO_DeInit, (int)sFLASH_SPI, NULL, 0, 0);

  /*!< sFLASH_SPI Periph clock disable ****************************************/
  HalDrvRccIOCtrl(eRCC_IO_SPI_Clock, eRCC_Clock_SPI4, NULL, 0, HAL_DISABLE);
...
#endif //#ifdef USE_STDPERIPH_DRIVER
