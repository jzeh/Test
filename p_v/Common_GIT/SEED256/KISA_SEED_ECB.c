...
#define Trace(...)  GITDebug(DEBUG_MODULES_KISA,__VA_ARGS__)

static const DWORD SS0[256] = {
...
/*********************************** END **************************************/
void DecyptCarDB_FromFlash(unsigned char* pDestAddress, DWORD dwSrcAddress, DWORD dwTotalReadSize)
{
	DWORD 	nReadSize, dwDecyptLength = 0;
	BYTE 	arrTempBuff[ENCRYPT_DECRYPT_DEFAULT_SIZE];
	DWORD 	pdwRoundKey[32];	// Round keys for encryption or decryption
	BYTE 	pbUserKey[ENCRYPT_DECRYPT_DEFAULT_SIZE] = {'G', 'I', 'T', 'A', 'U', 'T', 'O', '-',
	                      							   'F', 'I', 'N', 'E', ' ', 'O', 'B', 'D'};	// User secret key

	nReadSize = ENCRYPT_DECRYPT_DEFAULT_SIZE;

	SEED_KeySchedKey(pdwRoundKey, pbUserKey);

	while ( dwTotalReadSize > 0 )
	{
		memset(arrTempBuff, 0x00, nReadSize);
        if ( HalDrvFlashReadByteCallByRef((uint32_t*)&dwSrcAddress, (BYTE*)arrTempBuff, nReadSize) == HAL_RETURN_SUCCESS )
		{
			SEED_Decrypt(arrTempBuff, pdwRoundKey);
			memcpy(pDestAddress+dwDecyptLength, arrTempBuff, nReadSize);
			dwDecyptLength += nReadSize;
			Trace(">");

			dwTotalReadSize -= (nReadSize);
			if ( dwTotalReadSize < 16 )
				nReadSize = dwTotalReadSize;
		}
	}
	Trace("\r\n");
}

//void DecyptData(unsigned char* pDestAddress, unsigned char* pSrcAddress, DWORD dwDecryptSize)
//{
//	DWORD 	nReadSize, dwDecyptLength = 0;
//	BYTE 	arrTempBuff[ENCRYPT_DECRYPT_DEFAULT_SIZE];
//	DWORD 	pdwRoundKey[32];	// Round keys for encryption or decryption
//	BYTE 	pbUserKey[ENCRYPT_DECRYPT_DEFAULT_SIZE] = {'G', 'I', 'T', 'A', 'U', 'T', 'O', '-',
//	                         							'S', 'E', 'T', 'B', 'T', 'M', 'A', 'C'};	// User secret key
//
//	if ( dwDecryptSize > ENCRYPT_DECRYPT_DEFAULT_SIZE ) 	nReadSize = ENCRYPT_DECRYPT_DEFAULT_SIZE;
//	else													nReadSize = dwDecryptSize;
//
//
//	SEED_KeySchedKey(pdwRoundKey, pbUserKey);
//
//	while ( dwDecryptSize > 0 )
//	{
//		memset(arrTempBuff, 0x00, nReadSize);
//		memcpy(arrTempBuff, pSrcAddress, nReadSize);
//		SEED_Decrypt(arrTempBuff, pdwRoundKey);
//		memcpy(pDestAddress+dwDecyptLength, arrTempBuff, nReadSize);
//		dwDecyptLength += nReadSize;
//		Trace(">");
//
//		dwDecryptSize -= (nReadSize);
//		if ( dwDecryptSize < 16 )
//			nReadSize = dwDecryptSize;
//	}
//	Trace("\r\n");
//}

//void EncryptData(unsigned char* pDestAddress, unsigned char* pSrcAddress, DWORD dwEncryptSize)
//{
//	DWORD 	nReadSize, dwDecyptLength = 0;
//	BYTE 	arrTempBuff[ENCRYPT_DECRYPT_DEFAULT_SIZE];
//	DWORD 	pdwRoundKey[32];	// Round keys for encryption or decryption
...
//
//
//	if ( dwEncryptSize > ENCRYPT_DECRYPT_DEFAULT_SIZE ) 	nReadSize = ENCRYPT_DECRYPT_DEFAULT_SIZE;
//	else													nReadSize = dwEncryptSize;
//
//	SEED_KeySchedKey(pdwRoundKey, pbUserKey);
//
//	while ( dwEncryptSize > 0 )
//	{
//		memset(arrTempBuff, 0x00, nReadSize);
//		memcpy(arrTempBuff, pSrcAddress, nReadSize);
//		SEED_Encrypt(arrTempBuff, pdwRoundKey);
//		memcpy(pDestAddress+dwDecyptLength, arrTempBuff, nReadSize);
//		dwDecyptLength += nReadSize;
//		Trace(">");
//
//		dwEncryptSize -= (nReadSize);
//		if ( dwEncryptSize < 16 )
//			nReadSize = dwEncryptSize;
//	}
//	Trace("\r\n");
//}

//void EncryptTestmain()
//{
//	/*
//	Test Vector for SEED : LITTLE_ENDIAN
//
//
...
//
//	K1,0  : 7C8F8C7E, K1,1  : C737A22C
//	K2,0  : FF276CDB, K2,1  : A7CA684A
//	K3,0  : 2F9D01A1, K3,1  : 70049E41
//	K4,0  : AE59B3C4, K4,1  : 4245E90C
//	K5,0  : A1D6400F, K5,1  : DBC1394E
//	K6,0  : 85963508, K6,1  : 0C5F1FCB
//	K7,0  : B684BDA7, K7,1  : 61A4AEAE
//	K8,0  : D17E0741, K8,1  : FEE90AA1
//	K9,0  : 76CC05D5, K9,1  : E97A7394
...
//
//	DWORD pdwRoundKey[32];														// Round keys for encryption or decryption
////	BYTE 	pbUserKey[16] = {'G', 'I', 'T', 'A', 'U', 'T', 'O', '-',
////	                         'F', 'I', 'N', 'E', ' ', 'O', 'B', 'D'};	// User secret key
////	BYTE pbData[16]    = {0x23, 0x48, 0x57, 0x53, 0x45, 0x54, 0x2C, 0x30, 		// input plaintext to be encrypted
////                        0x78, 0x30, 0x36, 0x2C, 0x37, 0x2E, 0x30, 0x30};
//
...
//	Trace ("\n\nRound Key  : \r\n");
//	for (i=0;i<16;i++) {
//		Trace("K%2d,0 : %08X\t", i+1, pdwRoundKey[2*i]);
